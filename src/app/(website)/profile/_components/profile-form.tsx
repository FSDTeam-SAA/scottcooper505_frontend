...
const ProfileInfo = () => {
  ...
  const token = (session?.data?.user as { accessToken: string })?.accessToken;
  const getProfile = useProfileQuery(token as string);
  const profileInfoUpdate = useProfileInfoUpdate(token as string);
  ...
  useEffect(() => {
    if (getProfile?.data?.data) {
      form.reset({
        fullName: getProfile.data.data.name || "",
        email: getProfile.data.data.email || "",
        phoneNumber: getProfile.data.data.phone || "",
        city: getProfile.data.data.address?.cityState || "",
        country: getProfile.data.data.address?.country || "",
      });
    }
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
            ...
            <Button
              ...
              type="submit"
            >
              Save {profileInfoUpdate.isPending && <Loader2 className="animate-spin" />}
            </Button>
          </form>
        </Form>
      </div>
